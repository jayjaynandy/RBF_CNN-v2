...
#%%
    def train(self, model, data, model_path, existing_path = None):
        ...
        # adv_perturbation
        diffgen1 = ImageDataGenerator(
                horizontal_flip=True
                )

        dataList1 = np.load("bim-05.npz")
        ...
        def flow_noisy(train_gen):
            ...
            while True:
                ...
                x3 = x0 + in_rand
                x3 = np.clip(x3, lower, upper)
                
                xf = np.concatenate((x0, x1, x3), axis=0)
                yf = np.concatenate((y, y, y), axis=0)        
                yield (xf, yf)
