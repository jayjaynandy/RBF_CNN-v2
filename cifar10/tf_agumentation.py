...
def random_brightness(x, brightness_range):
  ...
  x = array_to_img(x*255)
  
  ############### brightness ###########################
  x = ImageEnhance.Brightness(x)  
  x = x.enhance(np.random.uniform(brightness_range[0], brightness_range[1]))
...
class NumpyArrayIterator(Iterator):
  ...
  def _get_batches_of_transformed_samples(self, index_array):
    ...
    batch_x_nrml = np.zeros(tuple([len(index_array)] + list(self.x.shape)[1:]), dtype=K.floatx())
    
    for i, j in enumerate(index_array):
      batch_x_nrml[i] = self.x[j]
      x = self.x[j]
    ...
    return batch_x_nrml, batch_x, batch_y
